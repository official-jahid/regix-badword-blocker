/**
 * 🔥 REGIX GOD MODE — Full-Stack Dashboard Server v3.0
 * ======================================================
 * Complete backend with:
 * - JWT-based authentication
 * - User management (CRUD)
 * - Bot data CRUD (words, violations, permissions, config)
...
// ─── Bot data cache ──────────────────────────────────────────────────
let botData = {
  config: readJSON(join(DATA_DIR, "config.json")) || {},
  words: readJSON(join(DATA_DIR, "words.json")) || {
    badWords: [],
    whiteListWords: [],
  },
  violations: readJSON(join(DATA_DIR, "violations.json")) || {},
  permissions: readJSON(join(DATA_DIR, "permissions.json")) || {
    ignoredChannels: [],
    allowedUsers: [],
  },
};
...
function reloadBotData() {
  botData.config = readJSON(join(DATA_DIR, "config.json")) || botData.config;
  botData.words = readJSON(join(DATA_DIR, "words.json")) || botData.words;
  botData.violations =
    readJSON(join(DATA_DIR, "violations.json")) || botData.violations;
  botData.permissions =
    readJSON(join(DATA_DIR, "permissions.json")) || botData.permissions;
}
...
function saveBotData(type) {
  ...
  writeJSON(paths[type], botData[type]);
...
app.post("/api/bot/words/bad", isAuth, isAdminOrOwner, (req, res) => {
  ...
  saveBotData("words");
  addLog("word_add", `🚫 Added bad word: "${word}"`, req.user.username, req.ip);
...
app.get("/api/bot/status", isAuth, (req, res) => {
  res.json({
    ...
    commands: [
      ...
      { name: "reset", icon: "🔄", description: "Reset strikes for a user ✨" },
      {
        name: "manage",
        icon: "⚙️",
...
app.get("/api/logs", isAuth, (req, res) => {
  ...
  res.json({ logs: page, total, limit: parseInt(limit) || 100, offset: start });
});
...
app.get("/api/logs/export", isAuth, (req, res) => {
  ...
  if (format === "csv") {
    ...
    res.setHeader("Content-Disposition", "attachment; filename=regix-logs.csv");
    return res.send(csv.join("\n"));
  }

  res.setHeader("Content-Type", "application/json");
  res.setHeader("Content-Disposition", "attachment; filename=regix-logs.json");
...
async function start() {
  await seedAdmin();
  addLog("server_start", "🚀 Dashboard server started", "system");
  app.listen(PORT, () => {
    console.log(`
╔══════════════════════════════════════════╗
